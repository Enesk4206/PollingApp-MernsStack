...
const OptionInput = ({ optionList, setOptionList }) => {
    ...
    const handleAddOption = () => {
        if (option.trim() && option.length < 4) {
            setOptionList([...optionList, option.trim()]);
            setOption("");
        }
    }
    //Func to handle deleting an option
    ...
    return (
        <div>
            {optionList.map((item, index) => (
                ...
                <div key={`${item}-${index}`} className='flex justify-between bg-gray-200/80 px-4 py-2 rounded-md mb-3'>
